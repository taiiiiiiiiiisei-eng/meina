...
def main() -> int:
    ...
    setup_gap_week_route = route_command(
        "今週の予定設定不足を教えて",
        {"confidence": 0.10},
    )
    assert setup_gap_week_route is not None
    assert setup_gap_week_route["kind"] == "reminder_setup_gaps"
    assert setup_gap_week_route["query"] == {
        "scope": "week",
        "summary": False,
    }

    setup_gap_month_count_route = route_command(
        "今月の設定不足予定は何件？",
        {"confidence": 0.10},
    )
    assert setup_gap_month_count_route is not None
    assert setup_gap_month_count_route["kind"] == "reminder_setup_gaps"
    assert setup_gap_month_count_route["query"] == {
        "scope": "month",
        "summary": True,
    }

    setup_gap_all_route = route_command(
        "予定準備不足をチェックして",
        {"confidence": 0.10},
    )
    assert setup_gap_all_route is not None
    assert setup_gap_all_route["kind"] == "reminder_setup_gaps"
    assert setup_gap_all_route["query"] == {
        "scope": "all",
        "summary": False,
    }
    ...
    assert category_duration_week_route is not None
    assert category_duration_week_route["kind"] == "reminder_duration_total"
    assert category_duration_week_route["query"] == {
        "scope": "week",
        "category": "学校",
    }

    location_duration_month_route = route_command(
        "自習室で今月の予定時間合計",
        {"confidence": 0.10},
    )
    assert location_duration_month_route is not None
    assert location_duration_month_route["kind"] == "reminder_duration_total"
    assert location_duration_month_route["query"] == {
        "scope": "month",
        "location": "自習室",
    }
    ...
    assert missing_duration_route["query"] is None

    missing_duration_week_route = route_command(
        "今週の所要時間未設定の予定を教えて",
        {"confidence": 0.10},
    )
    assert missing_duration_week_route is not None
    assert missing_duration_week_route["kind"] == "reminder_missing_duration"
    assert missing_duration_week_route["query"] == "week"

    missing_duration_month_route = route_command(
        "今月の所要時間がない予定を教えて",
        {"confidence": 0.10},
    )
    assert missing_duration_month_route is not None
    assert missing_duration_month_route["kind"] == "reminder_missing_duration"
    assert missing_duration_month_route["query"] == "month"
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                duration_today = meina_reminders.add_reminder(
                    "今日の単発",
                ...
                duration_daily_timed = meina_reminders.add_reminder(
                    "毎日の時間あり",
                ...
                duration_month_once = meina_reminders.add_reminder(
                    "今月だけの単発",
                ...
                weekly_items = meina_reminders.remaining_scope_reminders(
                    duration_scope_now,
                    scope="week",
                )
                school_weekly = meina_reminders.filter_reminders_by_metadata(
                    weekly_items,
                    category="学 校",
                )
                assert meina_reminders.reminder_duration_summary(
                    school_weekly
                ) == {
                    "total_minutes": 210,
                    "timed_count": 6,
                ...
                classroom_weekly = meina_reminders.filter_reminders_by_metadata(
                    weekly_items,
                    location="教 室",
                )
                assert meina_reminders.reminder_duration_summary(
                ...
                monthly_items = meina_reminders.remaining_scope_reminders(
                    duration_scope_now,
                    scope="month",
                )
                study_room_monthly = meina_reminders.filter_reminders_by_metadata(
                    monthly_items,
                    location="自 習 室",
                )
                assert meina_reminders.reminder_duration_summary(
                ...
                assert len(
                    meina_reminders.reminders_missing_duration(weekly_items)
                ) == 5
                assert len(
                ...
                weekly_setup_gaps = meina_reminders.reminder_setup_gaps(
                    weekly_state_items
                )
                assert len(weekly_setup_gaps) == 2
                weekly_gap_map = {
                    entry["item"]["text"]: entry["missing"]
                    for entry in weekly_setup_gaps
                }
                assert weekly_gap_map == {
                    "今日の単発": ["メモ", "事前通知"],
                    "毎日の時間なし": ["所要時間", "メモ", "事前通知"],
                }

                monthly_setup_gaps = meina_reminders.reminder_setup_gaps(
                    monthly_state_items
                )
                assert len(monthly_setup_gaps) == 2
                assert len({
                    entry["item"]["id"]
                ...
                invalid_pre_notify_gap = meina_reminders.reminder_setup_gaps([
                    {
                        "id": "setup-invalid-notify",
                        "text": "不正事前通知",
                        "due_at": "2051-03-16T10:00:00+09:00",
                        "duration_minutes": 30,
                        "category": "確認",
                        "location": "自宅",
                        "note": "確認済み",
                        "notify_before_minutes": 1441,
                    },
                ])
                assert invalid_pre_notify_gap[0]["missing"] == ["事前通知"]
            finally:
