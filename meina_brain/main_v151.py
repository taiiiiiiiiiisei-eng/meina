...
def get_topic():
    ...
    for history in reversed(conversation_history):
        ...
        if user_text not in context_words:
            return history
...
print("会話履歴：", len(conversation_history))
...
while True:
    ...
    if text in context_words:
        ...
        previous_user = topic["user"]
        previous_meina = topic["meina"]
        ...
        if text in [
            ...
            reply = (
                f"「{previous_user}」についてですね。"
                f"さっき私は「{previous_meina}」"
                "と答えました。"
            )
        ...
        # それ
        reply = (
            f"さっきの話は「{previous_user}」ですね。"
            f"私の返事は「{previous_meina}」でした。"
        )
