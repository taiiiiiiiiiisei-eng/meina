...
# よく使う日本語の質問・接続表現を「1つの意味単位」として扱う。
# 形態素解析器に依存せず、今までの自作トークナイザーを強化する方式。
PHRASE_WORDS = [
    "なんで", "どうして", "なぜ",
    "どんな", "いつ", "どこ", "だれ", "誰",
    "どうやって", "どういう",
...
def tokenize(text):
    ...
    # 半角スペースで明示的に区切られている入力は、各部分を再解析する。
    chunks = text.split()
    if len(chunks) > 1:
        result = []
        for chunk in chunks:
            result.extend(tokenize(chunk))
        return result
    ...
    while remaining:
        ...
        for word in candidates:
            if remaining.startswith(word):
                normalized = NORMALIZE_WORDS.get(word, word)
...
print("v3.6 単語・文脈・意味・語順・単語役割・役割推論・役割一般化・文章意図推論・日本語フレーズ・文章構造解析・対象動作抽出開始...")
...
def fallback_role_affinity(word):
    """未登録語でも、既知の単語断片から役割を推定する。

    まだ語彙にない語を完全に理解するわけではないが、
    既知語を含む場合は、その役割傾向を組み合わせて推定する。
    """
    if word in word_to_id:
        return word_role_affinity(word).copy()

    candidates = []
    # 長い既知語を優先して、未知語の中に含まれる既知語を探す。
    for known in sorted(word_to_id.keys(), key=len, reverse=True):
        if len(known) >= 2 and known in word:
            candidates.append(known)

    if candidates:
        values = [word_role_affinity(w) for w in candidates[:5]]
        result = np.mean(values, axis=0)
        total = result.sum()
        return result / total if total > 0 else np.ones(3) / 3.0
...
def predict_word_role(word, slot_index=None):
    ...
    0=主体/話題、1=対象/状態、2=動作/結果。
    """
    affinity = fallback_role_affinity(word).astype(float)
...
PARTICLES = {"は", "が", "を", "に", "へ", "で", "と", "の", "も", "から", "まで", "より", "って"}
QUESTION_PHRASES = {"なんで", "どうして", "なぜ", "どんな", "いつ", "どこ", "だれ", "誰", "どうやって", "どういう"}
ACTION_WORDS = {"開く", "起動", "検索", "実行", "閉じる", "使う", "書く", "教えて", "調べる", "遊ぶ", "プレイ"}
STATE_WORDS = {"楽しい", "面白い", "好き", "暇", "元気", "賢い"}

def parse_sentence_structure(text):
    """助詞を捨てずに文章構造を読む。

    v3.5では、助詞そのものを役割単語にはせず、
    「何を」「何が」「どこで」などの関係を表す構造情報として利用する。
    """
    raw_words = [w for w in tokenize(text) if w and w not in PUNCTUATION]
    content = []
    relations = []
    ...
    for word in raw_words:
        if word in PARTICLES:
            ...
            continue
        if word in QUESTION_PHRASES and not content:
            relations.append(("疑問", word))
            continue
        content.append(word)
        if pending_particle:
            relations.append(("助詞", pending_particle))
            pending_particle = None
    ...
    for i, word in enumerate(content):
        ...
        if i > 0:
            # tokenize上で直前に現れた助詞を確認する。
            for j in range(len(raw_words) - 1, -1, -1):
                if raw_words[j] == word:
                    if j > 0 and raw_words[j - 1] in PARTICLES:
                        previous_particle = raw_words[j - 1]
                    break
        if previous_particle in {"を", "に", "へ", "で", "と"} and word not in ACTION_WORDS:
            role = "対象/状態"

        roles.append((word, role))

    return raw_words, content, roles, relations



def extract_action_frame(text):
    """文章から「対象」と「動作」を分離して取り出す。

    v3.6では、助詞と役割情報を使って、PC操作などにつながる
    基本フレームを「対象 + 動作」として構造化する。
    """
    raw_words, content_words, structural_roles, relations = parse_sentence_structure(text)
    ...
    if action_index is not None:
        # 「を」「に」「へ」「で」「と」の直前にある内容語を対象として優先。
        action_raw_index = raw_words.index(action) if action in raw_words else None
        if action_raw_index is not None:
            candidates = []
            for j in range(action_raw_index - 1, -1, -1):
                if raw_words[j] in PARTICLES:
                    if raw_words[j] in {"を", "に", "へ", "で", "と"} and j > 0:
                        candidate = raw_words[j - 1]
                        if candidate not in PARTICLES and candidate not in QUESTION_PHRASES:
                            candidates.append(candidate)
                            break
                elif raw_words[j] in PUNCTUATION:
                    continue
            if candidates:
                target = candidates[0]
                target_index = content_words.index(target) if target in content_words else None

        # 助詞で見つからなければ、動作の直前にある内容語を対象にする。
        if target is None and action_index > 0:
            for i in range(action_index - 1, -1, -1):
                candidate = content_words[i]
                if candidate not in QUESTION_PHRASES:
                    target = candidate
                    target_index = i
                    break

    # 実行可能な動作がない場合でも、状態・述語を補助情報として返す。
    predicate = None
    if action is None:
    ...
    # 助詞関係を簡潔にまとめる。
    particle_relations = [value for kind, value in relations if kind == "助詞"]
    question = any(kind == "疑問" for kind, _ in relations) or any(
        x in text for x in ["？", "?", "なぜ", "なんで", "どうして", "何"]
    )
    ...
    if action is not None and target is not None:
        ...
        confidence = 0.70
    elif predicate is not None and target is not None:
        confidence = 0.75
    else:
        confidence = 0.25
...
def infer_roles(text):
    """未知の文章について、助詞を構造として利用しながら各単語の役割を推定する。"""
    raw_words, words, structural_roles, relations = parse_sentence_structure(text)
    results = []
    role_names = ["主体/話題", "対象/状態", "動作/結果"]
    role_to_id = {name: i for i, name in enumerate(role_names)}

    for i, (word, structural_role) in enumerate(structural_roles):
        # 構造情報を優先しつつ、学習済み単語役割も混ぜる。
        slot = 0 if structural_role == "主体/話題" else 2 if structural_role == "動作/結果" else 1
        learned = predict_word_role(word, slot)
        structural = np.zeros(3, dtype=float)
        structural[role_to_id[structural_role]] = 1.0
        probs = 0.70 * structural + 0.30 * learned
        probs /= max(probs.sum(), 1e-9)
        best = int(np.argmax(probs))
        results.append((word, role_names[best], float(probs[best]), probs))
...
def _intent_keyword_score(text, keywords):
    score = 0.0
...
def infer_intent(text):
    """
    文章の役割推定＋単語意味＋簡単なキーワード情報を統合して、
    現時点で最も妥当な「文章の意図」を推定する。

    これはv3.3時点では生成AIではなく、学習済みベクトルと
    役割知識を組み合わせた推論器。
    """
    clean = text.strip()
    words = [w for w in tokenize(clean) if w and w not in "？?！!。、，,"]

    if not words:
        return "不明", 0.0, [], []

    # 既存のニューラルネット分類。
    base_intent, base_conf = classify(clean)

    # 役割推定。
    role_results = infer_roles(clean)

    role_labels = [r[1] for r in role_results]

    # キーワードによる補助スコア。
    scores = {name: 0.0 for name in INTENT_NAMES}

    for name, keywords in INTENT_PROTOTYPES.items():
        scores[name] += _intent_keyword_score(clean, keywords)

    # 文章末尾の動作/結果を命令・依頼の材料にする。
    if role_results:
        last_role = role_results[-1][1]
        if last_role == "動作/結果":
            scores["命令"] += 0.8
            scores["依頼"] += 0.5

    # 疑問符や疑問語。
    if any(x in clean for x in ["？", "?", "なぜ", "なんで", "どうして", "何"]):
        scores["質問"] += 2.0

    # あいさつ・感謝は明確な表現を優先。
    if any(x in clean for x in ["こんにちは", "こんばんは", "おはよう", "やあ"]):
        scores["あいさつ"] += 3.0

    if any(x in clean for x in ["ありがとう", "助かった", "感謝"]):
        scores["感謝"] += 3.0

    # 意味ベクトルをプロトタイプと比較。
    semantic_scores = {}
    for name, prototype_words in INTENT_PROTOTYPES.items():
        sims = []
        for proto in prototype_words:
            if proto in word_to_id:
                s = semantic_similarity(clean, proto)
                sims.append((s + 1.0) / 2.0)
        semantic_scores[name] = float(np.mean(sims)) if sims else 0.0

    # ベクトル類似度を小さく補助する。
    for name in scores:
        scores[name] += semantic_scores.get(name, 0.0) * 0.8

    # 既存分類結果も少し残す。
    if base_intent in scores:
        scores[base_intent] += 0.6 * base_conf

    best_intent = max(scores, key=scores.get)
    raw = scores[best_intent]
    total = sum(max(v, 0.0) for v in scores.values())

    confidence = raw / total if total > 0 else 0.0

    return best_intent, float(confidence), role_results, semantic_scores


def explain_intent(text):
    """意図推論の結果を人間向けに表示する。"""
    intent, confidence, roles, semantic_scores = infer_intent(text)

    print("めいな：文章全体の意図を推論します。")
    print("文章：", text)
    print("推定意図：", intent)
    print("意図信頼度：", round(confidence, 4))

    frame = extract_action_frame(text)
    print("対象・動作フレーム：")
    print("  対象：", frame["target"] if frame["target"] else "なし")
    print("  動作：", frame["action"] if frame["action"] else "なし")
    if frame["predicate"]:
        print("  状態・述語：", frame["predicate"])
    print("  フレーム信頼度：", round(frame["confidence"], 4))

    if roles:
        print("単語役割：")
        for word, role, role_conf, probs in roles:
            print(
                "  ", word, "→", role,
                "信頼度：", round(role_conf, 4)
            )
...
while True:
    ...
    if text.startswith("意図推論 "):

        target = text[5:].strip()

        if not target:
            print("めいな：推論する文章を入力してください。例：意図推論 メモ帳 開く")
            continue

        explain_intent(target)
        continue
    ...
    if text.startswith("フレーム解析 "):

        target_text = text[7:].strip()
        if not target_text:
            print("めいな：解析する文章を入力してください。例：フレーム解析 メモ帳を開いて")
            continue

        frame = extract_action_frame(target_text)

        print("めいな：文章から対象と動作を分離します。")
        print("文章：", target_text)
        print("対象：", frame["target"] if frame["target"] else "なし")
        print("動作：", frame["action"] if frame["action"] else "なし")
        if frame["predicate"]:
            print("状態・述語：", frame["predicate"])
        print("助詞関係：", frame["particles"] if frame["particles"] else "なし")
        print("疑問文：", "はい" if frame["question"] else "いいえ")
        print("フレーム信頼度：", round(frame["confidence"], 4))
        print("めいな：この結果を『対象＋動作』の基本フレームとして扱えます。")
        continue

    if text.startswith("構造解析 "):

        target = text[5:].strip()
        raw_words, content_words, structural_roles, relations = parse_sentence_structure(target)

        if not content_words:
            print("めいな：解析する文章を入力してください。")
            continue

        print("めいな：助詞を使って文章構造を解析します。")
        print("文章：", target)
        print("トークン：", raw_words)
        print("意味単位：", content_words)
        ...
        print("役割：")
        for word, role in structural_roles:
            print("  ", word, "→", role)

        print("めいな：助詞は単語としてではなく、単語同士の関係として扱いました。")
        continue

    if text.startswith("役割一般化 "):

        target = text[6:].strip()
        words = [w for w in tokenize(target) if w and w not in "？?！!。、，,"]

        if not words:
            print("めいな：推定する文章を入力してください。")
            continue

        print("めいな：学習済みの役割知識から、新しい文章の役割を一般化します。")
        print("文章：", target)
        ...
        for i, word in enumerate(words):
            ...
            best = int(np.argmax(probs))
            source = "学習済み語彙" if word in word_to_id else "既知語から一般化"
            print(word, "→", labels[best],
                  "信頼度：", round(float(probs[best]), 4),
                  "推定元：", source,
                  "分布：", np.round(probs, 3))
