...
print("めいなブレイン v2.7 起動")
print("文章意味・語順・役割スロットを独立学習するシステム：ON")
print("")
...
print("v2.7 単語・文脈・意味・語順・役割スロット学習開始...")
...
def role_prototypes():
    """コーパス中の位置から役割の代表ベクトルを作る。
    先頭＝主体/話題、中央＝対象/状態、末尾＝動作/結果として、
    位置ごとの単語ベクトルを平均する。
    """
    slots = [[], [], []]
...
def role_score(text):
    """文章内の各単語が、学習した役割スロットにどれだけ合うかを返す。"""
    words = [w for w in tokenize(text) if w in word_to_id]
    if not words:
        return np.zeros(3)

    prototypes = role_prototypes()
    scores = np.zeros(3)
    ...
    for i, word in enumerate(words):
        ...
        vec = normalize_vector(word_vectors[word_to_id[word]])
        scores[slot] = cosine_similarity(vec, prototypes[slot])

    return scores


def role_comparison_score(text_a, text_b):
    """役割スロットそのものを比較する。順番が逆なら別の役割配置になる。"""
    ...
    def slot_vectors(words):
        ...
        for i, word in enumerate(words):
            if len(words) == 1:
                ...
                slot = 1
            slots[slot] = word_vectors[word_to_id[word]]
        return slots

    a = slot_vectors(words_a)
    b = slot_vectors(words_b)

    # 同じスロットに入った単語の意味が近いかを基本評価。
    # さらに各単語が自分の役割prototypeに合う度合いも加える。
    direct_scores = []
    prototype_scores = []

    for slot in range(3):
        if a[slot] is not None and b[slot] is not None:
            direct_scores.append(cosine_similarity(a[slot], b[slot]))
            prototype_scores.append(
                0.5 * cosine_similarity(a[slot], prototypes[slot])
                + 0.5 * cosine_similarity(b[slot], prototypes[slot])
            )

    if not direct_scores:
        return 0.0

    direct = float(np.mean(direct_scores))
    proto = float(np.mean(prototype_scores)) if prototype_scores else 0.0

    # v2.7：役割配置を独立して評価する。
    # 同じ単語の逆順では同一スロットに別単語が入りやすいため、
    # 役割の一致を明示的に下げる。
    return max(-1.0, min(1.0, 0.55 * direct + 0.45 * proto))
...
def role_sentence_vector(text):
    ...
    return normalize_vector(result / length)
...
def role_similarity(text_a, text_b):
    return cosine_similarity(
        role_sentence_vector(text_a),
        role_sentence_vector(text_b)
    )
