...
MARKER = "# MEINA_REMINDER_INTEGRATION_V1"
BACKUP = ROOT / "meina_agent.py.backup_before_reminder_v1"

IMPORT_BLOCK = "from meina_reminders import add_reminder, complete_reminder, due_reminders\nfrom meina_reminder_parser import parse_reminder_command\n"
...
def main() -> int:
    ...
    if "def _execute_routed_command_before_reminder" not in text:
        match = re.search(r"^def execute_routed_command\(route\):", text, re.MULTILINE)
        if not match:
            raise RuntimeError("execute_routed_command not found")
        text = text[:match.start()] + "def _execute_routed_command_before_reminder(route):" + text[match.end():]

    wrapper = '''\n\ndef execute_routed_command(route):\n    if route.get("kind") == "reminder":\n        return _execute_reminder(route)\n    return _execute_routed_command_before_reminder(route)\n'''
    insert_before = "\n\ndef execute_action(frame):"
    if insert_before not in text:
        raise RuntimeError("execute_action anchor not found")
    text = text.replace(insert_before, HANDLER + wrapper + insert_before, 1)
    ...
    print("Reminder voice integration applied")
    return 0
