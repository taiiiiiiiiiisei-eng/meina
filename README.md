...
- 予定の設定不足チェック（「今週の予定設定不足を教えて」「今月の設定不足予定は何件？」「予定準備不足をチェックして」）。所要時間・カテゴリ・場所・メモ・事前通知の未設定項目をまとめて表示し、定期予定は期間内へ投影しても同じ予定IDを1件として集計。「今週の設定不足内訳を教えて」で不足項目ごとの件数、「今週の完全設定済み予定を教えて」「今月の設定完了予定は何件？」で5項目がすべて設定済みの予定も確認可能。「今週の予定準備状況を教えて」「今月の設定完了率を教えて」では総予定数・完全設定数・設定不足数・設定完了率・不足が多い項目を一度に確認可能
