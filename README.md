...
- 予定追加時の重なり警告、重要予定の設定・解除（「宿題の予定を重要にして」）と重要予定だけの確認（「重要な予定を教えて」「今週の重要予定を教えて」「今月の重要予定は何件？」）。今日/明日/今週/今月の期間指定に対応し、定期予定も投影して確認可能
- 一時停止中予定の確認（「明日の一時停止中の予定を教えて」「今週の一時停止中の予定は何件？」）。一時停止中の定期予定も期間内へ投影して一覧・件数を確認可能
- 事前通知設定の有無で予定を確認（「今週の事前通知あり予定を教えて」「今月の事前通知未設定予定は何件？」）。今日/明日/今週/今月に対応し、定期予定も期間内へ投影
- 繰り返し・定期予定の確認（「明日の繰り返し予定を教えて」「今月の定期予定は何件？」）。毎日・平日・毎週・毎月の各回を期間内へ投影し、一時停止中でも設定確認用の一覧には含める
- 予定の設定不足チェック（「今週の予定設定不足を教えて」「今月の設定不足予定は何件？」「予定準備不足をチェックして」）。所要時間・カテゴリ・場所・メモ・事前通知の未設定項目をまとめて表示し、定期予定は期間内へ投影しても同じ予定IDを1件として集計。「今週の設定不足内訳を教えて」で不足項目ごとの件数、「今週の完全設定済み予定を教えて」「今月の設定完了予定は何件？」で5項目がすべて設定済みの予定も確認可能。「今週の予定準備状況を教えて」「今月の設定完了率を教えて」では総予定数・完全設定数・設定不足数・設定完了率・不足が多い項目を一度に確認可能。さらに「今週の重要予定の設定不足を教えて」「今月の重要予定の設定不足は何件？」「今週の重要予定の準備状況を教えて」で重要予定だけに絞った準備チェックにも対応
...
- 予定メモの追加・確認・削除（「宿題の予定にメモを追加して『英語のワーク30ページ』」「宿題の予定のメモを教えて」「宿題の予定のメモを消して」）。「今週のメモ付き予定を教えて」「明日のメモ未設定の予定を教えて」「今月のメモ付き予定は何件？」のようなメモ有無での一覧・件数確認にも対応し、今週/今月は定期予定を未来投影。一覧では本文を出さず「メモあり」と表示し、完了履歴にはその時点のメモを保持
