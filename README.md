...
- 予定時間の合計確認（「今日の予定時間合計」「今週の予定時間合計」「今月何時間予定入ってる？」）に加え、カテゴリ/場所で絞った時間集計（「学校カテゴリの今週の予定時間合計」「自習室で今月の予定時間合計」）、カテゴリ/場所ごとの時間内訳（「今週のカテゴリ別予定時間」「今月の場所別予定時間」）にも対応。各内訳は合計時間と所要時間未設定件数を表示し、今週/今月は定期予定を未来投影。「今日/明日/今週/今月の所要時間未設定の予定」で期間指定一覧も確認可能。今日残りの空き時間（「今日あとどれくらい空いてる？」）にも対応
...
- 予定カテゴリの設定・解除（「宿題の予定を学校カテゴリにして」）、カテゴリ別一覧（「学校カテゴリの予定を教えて」）に加え、期間＋カテゴリ指定（「今週の学校カテゴリの予定を教えて」「学校カテゴリの今月の予定を教えて」）にも対応。今日/明日/今週/今月のカテゴリ別件数も確認可能
- 予定詳細の確認（「宿題の予定の詳細を教えて」「18時の宿題の予定を詳しく教えて」）。日時・所要時間・繰り返し・カテゴリ・重要度・場所・事前通知・一時停止状態・メモをまとめて表示。同名予定が複数ある場合は勝手に選ばず候補時刻を案内
...
- 場所ベースの予定確認（「図書館での予定を教えて」「場所が図書館の予定を教えて」）に加え、期間＋場所指定（「明日の図書館での予定を教えて」「場所が図書館の今月の予定を教えて」）にも対応。場所未設定一覧（「場所未設定の予定を教えて」）、今日/明日/今週/今月の場所別件数も確認可能
